...
error_log("🔍 REQUEST - MAC: '$mac_norm', IP: '$ip'");
    ...
    if (!$hayErrores) {
        try {
            ...
            if ($check_radcheck->num_rows > 0) {
                ...
                $conn->commit();
                error_log("✅ MAC ya estaba registrada: $mac_norm");
                // ✅ RETORNAR LA RESPUESTA MÁGICA PARA ARUBA
                header('Content-Type: text/plain');
                die('InstantOn.Acknowledge');
            }
            $check_radcheck->close();
            ...
            $stmt_clients->close();
            error_log("✅ CLIENTE INSERTADO - MAC: $mac_norm");
            ...
            // ✅ RETORNAR LA RESPUESTA MÁGICA PARA ARUBA
            // Esta es la clave: Aruba verifica esta respuesta exacta
            header('Content-Type: text/plain');
            die('InstantOn.Acknowledge');
...
if ($mac_norm !== '') {
    try {
        ...
        if ($check->num_rows > 0) {
            ...
            error_log("ℹ️ MAC ya registrada, retornando acknowledge automático");
            
            // ✅ SI LA MAC YA ESTABA REGISTRADA, RETORNAR ACKNOWLEDGE
            header('Content-Type: text/plain');
            die('InstantOn.Acknowledge');
        }
        $check->close();
    ...
    <div class="form-container">
        ...
        <?php if ($mac_norm !== ''): ?>
            <div class="mac-display">
                <strong>📱 Tu Dispositivo:</strong><br>
                MAC: <?php echo $mac_norm; ?>
            </div>
            
            <div class="info-display">
                📝 Completa el formulario para conectar
            </div>
        ...
        <?php else: ?>
            <div class="error">
                ❌ No se detectó MAC<br>
                <small>Conéctate a la red Wi-Fi primero</small>
            </div>
        <?php endif; ?>
    </div>

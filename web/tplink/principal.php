...
/*
 TP-Link standalone:
   http://portal/principal.php?clientMac=AA:BB:CC:...&target=192.168.0.10
   - clientMac = MAC del cliente
   - target    = IP del AP

 TP-Link Omada:
   ...?clientMac=AA:BB:...&apMac=11:22:33:44:55:66&ssidName=...
   - clientMac = MAC del cliente
   - apMac     = MAC del AP

 Tus scripts antiguos:
   ?mac=...&ap_mac=...&ip=...
*/

# 1) nombres que manda TP-Link
$client_mac_raw = $_GET['clientMac'] ?? $_POST['clientMac'] ?? '';
$ap_mac_raw_tpl = $_GET['apMac']     ?? $_POST['apMac']     ?? '';
$ap_ip_tpl      = $_GET['target']    ?? $_POST['target']    ?? ''; // IP del AP en standalone
...
error_log("🔍 TP-LINK PARAMS → clientMac='{$client_mac_raw}', apMac='{$ap_mac_raw_tpl}', target='{$ap_ip_tpl}'");
error_log("🔍 PARÁMETROS FINALES - MAC_CLIENTE: '$mac_norm', IP Cliente: '$ip', AP_IP: '$ap_ip', AP_MAC: '$ap_norm'");
...
<body>
    ...
    <div class="form-container">
        ...
        <div class="mac-display">
            ...
            <?php echo $ap_norm !== '' ? $ap_norm : '— no llegó (envía ?ap_mac=xx:xx:xx:xx:xx:xx o usa Omada) —'; ?>
            <br><br>
